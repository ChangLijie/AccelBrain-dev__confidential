...
from open_webui.apps.model_handler.main import (
    app as model_handler_app,
)
...
class ChatCompletionMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        with open("./temp/reference.txt", "w", encoding="utf-8") as file:
            file.write(f"len : {len(citations)}.\n")
            for e in citations:
                try:
                    file.write(f"{e['source']['file']['filename']}.\n")
                except:
                    for file_info in e["metadata"]:
                        file.write(f"{file_info['name']}.\n")
        ...
        if len(contexts) > 0:
            ...
            if model["owned_by"] == "ollama":
                with open("./temp/context_string.txt", "w", encoding="utf-8") as file:
                    file.write(f"len : {len(contexts)}.\n")
                    for i in contexts:
                        file.write(f"l{i}.\n*****************************\n")
                    if isinstance(context_string, dict):
                        file.write(f"len : {len(context_string)}.\n")
                    file.write(
                        f"---------------------------\n{context_string}.---------------------------\n"
                    )
                body["messages"] = prepend_to_first_user_message_content(
                    rag_template(
                        template=retrieval_app.state.config.RAG_TEMPLATE,
        ...
        with open("./temp/body.txt", "w", encoding="utf-8") as file:
            file.write(f"{modified_body_bytes}.\n")
...
app.mount("/model_handler", model_handler_app)
